public class Test {
	...
	public static void main(String[] args) {
		Bayes_net sample = new Bayes_net();
		double[] a = {.5,.25,.25};
		sample.add_vertex("A",3, a);
		...
		double[][] ab = {{.5,.5},{0.4,0.6},{0.3,0.7}};
		double[][] ac = {{.7,.3},{0.2,0.8},{.5,.5}};
		double[][] cg = {{.8,.2},{.1,.9}};
		double[][] ce = {{.3,.7},{.6,.4}};
		double[][][] geh = {{{.05,.95},{.95,.05}},{{.95,.05},{.95,.05}}};
		double[][] bd = {{.9,.1},{.5,.5}};
		double[][][] def = {{{.01,.99},{.01,.99}},{{.01,.99},{.99,.01}}};
		sample.add_edge("A",3, "B",2,ab);
		sample.add_edge("A",3, "C",2,ac);
		...
		JoinTree test = sample.convertToJoinTree();
		System.out.println(test.makeConsistent());
		Vertex<String> E = new Vertex<>("A", 3);
		test.enter_evidence(E, 2);
